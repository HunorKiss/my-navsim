...
class CameraOnlyModel(nn.Module):
    def __init__(self, trajectory_sampling: TrajectorySampling, config: CameraOnlyConfig, hidden_dim=128, num_attention_heads=4):
        """
        ...
        # Replace with DINO
        # --- JAVÍTÁS 1: STABIL DINOv2 AZONOSÍTÓ HASZNÁLATA ---
        pretrained_model_name = "facebook/dinov3-vitl16-pretrain-lvd1689m"
        self._processor = AutoImageProcessor.from_pretrained(pretrained_model_name)
        
        # A DINOv3 nagy, használja az automatikus eszközleképezést
        self._model = AutoModel.from_pretrained(
            pretrained_model_name, 
            # device_map="auto", 
        )
        
        # A ViT kimeneti mérete: 4096
        vit_output_dim = self._model.config.hidden_size  # Méret: 4096
        
        # A fúziós dimenziókat kényszerítjük a 4096-ra az additív fúzióhoz
        ego_mlp_output_dim = vit_output_dim
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        cls_token, patch_embeddings = self._extract_vit_embedding(camera_input)  # cls_token: (B, d_model)
        ...
        # print(f"Aux tasks enabled: {self._aux_tasks_enabled}")

        # Optional agent prediction
        if self.training and self._aux_tasks_enabled:
            # A. Memória létrehozása (TÉRBELI PATCHES + Ego Status)
            # A memóriának tartalmaznia kell a térbeli patch-eket és az ego-állapotot (mint egy extra tokent)
            global_status_token = status_embedding.unsqueeze(1) # (B, 1, d_model)
            memory_for_detection = torch.cat([patch_embeddings, global_status_token], dim=1) # (B, patches+1, d_model)
            
            # B. Lekérdezések előkészítése
            queries = self.agent_queries.unsqueeze(0).repeat(batch_size, 1, 1)

            # C. Multihead Attention (TÉRBELI ÉRVELÉS)
            # A queries a patch-ekből és az ego-statusból épített memóriára figyel.
            attended_queries, _ = self.agent_attn(queries, memory_for_detection, memory_for_detection)
    ...
    def _extract_vit_embedding(self, image_tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        # 1. Get model’s device (Most a device_map="auto" miatt a modell automatikusan a GPU-n van)
        device = self._model.device # Megkapjuk a ViT tényleges eszközét (pl. cuda:0)
        inputs = self._processor(images=image_tensor, return_tensors="pt")
        inputs = {k: v.to(device) for k, v in inputs.items()}
        
        # --- Inferenciális mód (sebesség és stabilitás) ---
        with torch.inference_mode():
            outputs = self._model(**inputs)

        cls_token = outputs.last_hidden_state[:, 0, :]       # CLS token
        patch_embeddings = outputs.last_hidden_state[:, 1:, :]  # all other tokens
