...
class CameraOnlyAgent(AbstractAgent):
    ...
    def __init__(
        ...
        for param in self._camera_only_model._model.parameters():
    ...
    def initialize(self) -> None:
        ...
        # 1. Betöltjük a súlyokat a CPU-ra (ez a DDP standard gyakorlata)
        state_dict: Dict[str, Any] = torch.load(
            self._checkpoint_path, 
            map_location=torch.device("cpu") # MINDIG CPU, hogy a DDP oszthassa el!
        )["state_dict"]
        ...
        for k, v in state_dict.items():
            ...
            # FIGYELEM: A súlyok CPU-n vannak, ne mozgassuk át a .to(device) hívással!
            if model_key in model_state_dict:
                new_state_dict[model_key] = v 
                
        # 3. Betöltjük az állapotot
        # A DDP/Lightning ezután automatikusan minden paramétert a megfelelő GPU-ra fog mozgatni.
        self.load_state_dict(new_state_dict, strict=False)

        # 4. KRITIKUS: Nincs self.to_empty() vagy self.to(device) hívás.
        # A DDP feladata a modell eszközre helyezése.
